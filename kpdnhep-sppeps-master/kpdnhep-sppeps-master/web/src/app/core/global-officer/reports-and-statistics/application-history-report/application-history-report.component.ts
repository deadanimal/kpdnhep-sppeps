...
@Component({
  selector: 'app-application-history-report',
  templateUrl: './application-history-report.component.html',
  styleUrls: ['./application-history-report.component.scss']
})
export class ApplicationHistoryReportComponent implements OnInit {
  ...
  getChart2() {
    ...
    var series1 = createSeries("europe", "Europe");
    series1.hidden = false
    var series2 = createSeries("namerica", "North America");
    series2.hidden = true
    var series3 = createSeries("asia", "Asia-Pacific");
    series3.hidden = true
    var series4 = createSeries("lamerica", "Latin America");
    series4.hidden = true
    var series5 = createSeries("meast", "Middle-East");
    series5.hidden = true
    var series6 = createSeries("africa", "Africa");
    series6.hidden = true
  ...
  filterCharts() {
    ...
    const filteredTable = this.temp.filter(el => new Date(el.submitDate).getTime() >= startDate.getTime() && new Date(el.submitDate).getTime() <= endDate.getTime());
    // const temp = this.temp.filter(function (d) {
    //   return (
    //     (d.submitDate.indexOf(startDate) !== -1 || !startDate) 
    //   );
    // });
